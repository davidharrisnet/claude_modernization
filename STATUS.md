...
Used Claude to create a Spring Boot 4.1.1 backend (the model layer) in `master-antique-repair-claude/backend/postgresql/`. It connects to the PostgreSQL database, maps the migrated tables with JPA, and demonstrates the first-login password change every migrated user must make. See [docs/phase2/model/postgresql/MODEL_PLAN.md](docs/phase2/model/postgresql/MODEL_PLAN.md).
