...
Apply identically to `tools/dbmigrate/iteration1/` and `tools/dbmigrate/iteration2/` — they're separate copies of the same tooling, so the same edits happen twice (or make the change once and re-sync the copies, whichever this project's convention prefers by the time this is implemented):
...
- Update `docs/DATA_MIGRATION.md` §3 (iteration roadmap table) and §6 ("what's not built yet") once this lands — remove the "moving `sanitize.py` to run at export time" bullet, since for 1/2 the equivalent (sanitize before serialization) will be done, and note that it's specifically *not* the same code as iteration 3's (different mechanism, same policy outcome).

## Open question to resolve on the Windows side, not answered here

Should `Protect-SensitiveData`'s table/column list be hardcoded (matches iteration 3's approach, lowest risk, fastest to implement) or should this refactor be the moment the config-driven sensitive-column declaration (`docs/DATA_MIGRATION.md` §5.2.5) gets built for real? Recommend hardcoded first, generalize later — but that's a call worth making deliberately when picking this up, not defaulting into either direction.
