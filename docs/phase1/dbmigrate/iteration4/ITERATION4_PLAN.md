...
- **Command:** `tools\phase1\dbmigrate\iteration4\dbmigrate4.cmd <export|selftest|report|all> --target postgres`, from a plain Windows command prompt, with the legacy app's LocalDB database as the source. Deterministic; no AI in the loop. Exit codes: 0 ok, 1 selftest differences, 2 error, 3 refused.
- **Outputs (checked in):** `01-schema.sql`, `02-data-sanitized.sql`, `source-metadata.json`, in `tools/phase1/dbmigrate/iteration4/`; and `docs/phase1/dbmigrate/iteration4/MigrationExportReport4.docx`.
...
- **Self-contained:** own copy of the tooling; reads nothing from `tools/phase1/dbmigrate/iteration1|2|3/` at run time. Copy code out of iteration 2 once, at build time.
- Directories: `tools/phase1/dbmigrate/iteration4/` and `docs/phase1/dbmigrate/iteration4/`.
...
| 8 | Windows tooling | **Settled** | Own self-contained copy of the tooling under `tools/phase1/dbmigrate/iteration4/`, per the per-iteration convention. |
...
| 11 | Output file names | **Settled** | `01-schema.sql`, `02-data-sanitized.sql`, `source-metadata.json`, all in `tools/phase1/dbmigrate/iteration4/`. |
...
**Existing tooling to copy from (iteration 2, `tools/phase1/dbmigrate/iteration2/`):**
...
```
tools/phase1/dbmigrate/iteration4/
...
docs/phase1/dbmigrate/iteration4/
  ITERATION4_PLAN.md (this file)    ITERATION4.md (written after the real run)
  MigrationExportReport4.docx       OUTPUT, checked in
```

Config `migration.config.json` (paths relative to the repo root): `source` (server, database as in section 4), `outputDir: "tools/phase1/dbmigrate"`, `reportDir: "docs/phase1/dbmigrate/iteration4"`, target `postgres`: `dialect: "postgres"`, `outputSubdir: "iteration4"`, `sanitizeCredentials: true`, `caseInsensitiveUniqueIndexes: [{ "table": "Users", "column": "Name" }]`.
...
- Add `tools/phase1/dbmigrate/iteration4/.gitattributes` with `*.sql text eol=lf` and `*.json text eol=lf`. The metadata records SHA-256 hashes of the LF form and string literals may contain raw LF; git line-ending conversion on Windows would otherwise break both the transfer hash and the data. Verify after a fresh clone on Windows and Linux.
- **Hand-off to iteration 5** (manual, on Windows, after a successful export): copy the three files into `tools/phase1/dbmigrate/iteration5/input/` and commit them together with `tools/phase1/dbmigrate/iteration4/`. Iteration 5 reads only its own `input/` folder.
...
- The three files are copied into `tools/phase1/dbmigrate/iteration5/input/` and committed.
...
8. Write `README.md`, `docs/phase1/dbmigrate/iteration4/ITERATION4.md` (what actually happened, with real numbers), and update `docs/phase1/dbmigrate/DATA_MIGRATION.md` (§3 status, §8 and §9 document map) and the commands paragraph of `CLAUDE.md`.
