...
**Orchestrated from Windows.** The *target* database lives inside a Linux container, but the tool driving this iteration is still Windows PowerShell, needing a live SQL Server LocalDB connection and Docker Desktop — this iteration cannot be run from a Linux or macOS host. Only iteration 3 runs its tooling entirely on Linux; see [docs/phase1/dbmigrate/DATA_MIGRATION.md](../DATA_MIGRATION.md).
...
| Where the database file is created | `tools\phase1\dbmigrate\iteration1\masterantique.sqlite` on the PC | `/data/masterantique.sqlite` **inside the container**, and only there: it is never copied to Windows |
| Command | `tools\phase1\dbmigrate\iteration1\dbmigrate.cmd all --target sqlite` | `tools\phase1\dbmigrate\iteration2\dbmigrate.cmd all --target sqlite-linux` |
...
| Tooling | `tools\phase1\dbmigrate\iteration2\dbmigrate.cmd` from a normal command prompt; deterministic PowerShell, no AI in the loop |
...
The tool reads SQL Server's description of itself (tables, columns, keys, relationships, indexes, defaults, auto-number counters), works out the order in which tables must be built, reads every row and converts it to one agreed text form, and writes two files into `tools\phase1\dbmigrate\iteration2\`:
...
Files produced in `tools\phase1\dbmigrate\iteration2\` (gitignored: the data file contains password hashes): `01-schema.sql`, `02-data.sql`, `import-log.txt`, `verification-results.json`, `selftest-results.json`. `MigrationVerificationReport2.docx` and `SQLiteDatabaseGuide2.docx` are written to `docs\phase1\dbmigrate\iteration2\`. The database is only at `/data/masterantique.sqlite` in the container `mar-sqlite`; there is no `.sqlite` file on Windows.
...
| Check | Result |
|---|---|
| `01-schema.sql` and `02-data.sql` compared with `tools\phase1\dbmigrate\iteration1\` | Byte-identical |
