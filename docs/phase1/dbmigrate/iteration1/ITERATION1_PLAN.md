...
`tools\phase1\dbmigrate\dbmigrate.cmd` -> `powershell -NoProfile -ExecutionPolicy Bypass -File tools\phase1\dbmigrate\migration1\DbMigrate.ps1 %*`
```
...
## Configuration: `tools/phase1/dbmigrate/migration1/migration.config.json`
```
{ "source":  { "server": "(localdb)\\MSSQLLocalDB", "database": "aspnet-MasterAntiqueRepair-e93a6129-..." },
  "outputDir": "export",
  "targets": { "sqlite": { "dialect": "sqlite", "exe": "C:\\Apps\\sqlite-tools-win-x64-3530400\\sqlite3.exe", "diffExe": "C:\\Apps\\sqlite-tools-win-x64-3530400\\sqldiff.exe", "file": "tools/phase1/dbmigrate/iteration1/masterantique.sqlite" } } }
```
`--target` picks a block; `dialect` picks `dialects/<name>.ps1`. Unknown dialect -> exit 2 listing implemented ones.

## Architecture
```
tools/phase1/dbmigrate/dbmigrate.cmd
tools/phase1/dbmigrate/migration1/
...
## Outputs (`tools/phase1/dbmigrate/iteration1/`, gitignored - contains credential-equivalent password hashes)
...
## Files to touch
- New: everything under `tools/phase1/dbmigrate/migration1/` and `tools/phase1/dbmigrate/dbmigrate.cmd`.
...
2. From a plain **cmd.exe** prompt: `tools\phase1\dbmigrate\dbmigrate all --target sqlite` -> exit 0, all checks PASS, `.sqlite` opens in `sqlite3` and per-table counts match the source counts above.
