# Iteration 5: the migrated database in PostgreSQL, in Docker

Iteration 5 takes the MasterAntiqueRepair database, exported from SQL Server by iteration 4, loads it into a
...
1. **Checks the input.** The three files from iteration 4 (the schema, the data, and a record of the source database)
...
| [MigrationVerificationReport5.html](MigrationVerificationReport5.html) | The verification report: results, tables, structure, business summaries, rules, how to reproduce |
...
Claude Code follows the iteration's instructions (`tools/phase1/dbmigrate/iteration5/CLAUDE.md`): it runs the command
...
[tools/phase1/dbmigrate/iteration5/README.md](../../../../tools/phase1/dbmigrate/iteration5/README.md).

**The backend that uses this database** is not part of this iteration: it is Phase 2 code in the separate repository
`master-antique-repair-claude`. With this repository open in Claude Code, type `Run the backend demonstration.` (or
`Rebuild the backend.` to rebuild its code); Claude Code follows
[docs/phase2/model/MODEL_PLAN.md](../../../phase2/model/MODEL_PLAN.md). Repeating this iteration removes the
database login and network route the backend uses; the demonstration recreates them.

## Limits and known differences

- **Checked against a record, not the live source.** The comparison is with the record iteration 4 made at export. It
...
- [tools/phase1/dbmigrate/iteration5/CLAUDE.md](../../../../tools/phase1/dbmigrate/iteration5/CLAUDE.md): the detailed
  instructions for repeating and maintaining this iteration (written for Claude Code).
- [../iteration4/](../iteration4/): the export that produced this iteration's input.
- [../DATA_MIGRATION.md](../DATA_MIGRATION.md): the migration strategy and security policy for all iterations.
- [docs/phase2/model/MODEL_PLAN.md](../../../phase2/model/MODEL_PLAN.md): the Spring Boot backend built on this
  database.
