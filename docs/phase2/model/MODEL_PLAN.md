...
| The database it connects to | Iteration 5's `mar-postgres` container: [../../dbmigrate/iteration5/ITERATION5_PLAN.md](../../dbmigrate/iteration5/ITERATION5_PLAN.md) |
| How to reach that database (logins, network routes) | [../../dbmigrate/iteration5/PostgreSQLDatabaseGuide.html](../../dbmigrate/iteration5/PostgreSQLDatabaseGuide.html) |
...
Prerequisites: Java 21, Docker Engine, the `mar-postgres` container from iteration 5 (`tools/dbmigrate/iteration5/ingest.sh load` if it does not exist), and a clone of `master-antique-repair-claude`.
...
1. **Create the application login** (once; the guide, section 3). This makes `mar_app` with a password you choose:
   ...
   unset APP_PW RO_PW
   ```
   (`mar-roles.sql` is also kept at `tools/dbmigrate/iteration5/guide/mar-roles.sql`.)
...
**Where the code came from:** the entities, repositories, `LoginService` and demo classes are the tested sample project from the PostgreSQL database guide (`tools/dbmigrate/iteration5/guide/mar-db-client/`, package `com.masterantique.dbclient`), copied with the package renamed. New for the backend: `RejectingIdentityCheck`, the `demo` profile on the demo classes, `DatabaseCheck` (replacing the sample's console `ConnectionCheck`), the test suite, and the build and ignore files.
