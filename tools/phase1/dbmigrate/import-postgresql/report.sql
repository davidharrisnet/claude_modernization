-- import-postgresql report: renders MigrationVerificationReport.html from verification-results.json (and
...
<main>
<header>
<p class="eyebrow">MasterAntiqueRepair &middot; database migration &middot; import-postgresql</p>
<h1>Migration verification report</h1>
<p class="lede">The SQL Server export from export-postgresql, loaded into PostgreSQL in a Docker container on Linux and checked against the metadata recorded at export.</p>
</header>
'
...
<p>The same ten questions export-postgresql asked of SQL Server, answered by PostgreSQL queries and compared with the answers recorded from SQL Server.</p>'
...
<pre><code>tools/phase1/dbmigrate/import-postgresql/ingest.sh all --recreate   # load, verify, selftest, report
tools/phase1/dbmigrate/import-postgresql/ingest.sh verify           # re-check the running database
...
|| format('<dt>Exported</dt><dd>%s UTC, export tool commit <code>%s</code> (export-postgresql)</dd>',
     pg_temp.h(r.j -> 'sourceExport' ->> 'runTimeUtc'), pg_temp.h(r.j -> 'sourceExport' ->> 'toolGitCommit'))
|| '</dl></section>
<footer><p>Generated by <code>tools/phase1/dbmigrate/import-postgresql/ingest.sh report</code> from <code>verification-results.json</code> and <code>selftest-results.json</code>. Description: <code>docs/phase1/dbmigrate/import-postgresql/README.md</code>; instructions for Claude Code: <code>tools/phase1/dbmigrate/import-postgresql/CLAUDE.md</code>.</p></footer>
</main>
</body>
</html>'
FROM r;
