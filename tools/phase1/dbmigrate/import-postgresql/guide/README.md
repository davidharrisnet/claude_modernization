...
`docs/phase1/dbmigrate/import-postgresql/PostgreSQLDatabaseGuide.html` is generated from this folder. **Every code block in the guide
...
```
python3 tools/phase1/dbmigrate/import-postgresql/guide/build-guide.py
```
...
sed -e 's/^CONTAINER=.*/CONTAINER=mar-postgres-guide/' \
    -e "s|^INPUT_DIR=.*|INPUT_DIR=$PWD/tools/phase1/dbmigrate/import-postgresql/input|" \
    tools/phase1/dbmigrate/import-postgresql/ingest.conf.example > /tmp/guide.conf
tools/phase1/dbmigrate/import-postgresql/ingest.sh load --config /tmp/guide.conf
...
docker exec -i -e APP_PW -e RO_PW mar-postgres-guide psql -X -q -U masterantique -d masterantique \
  < tools/phase1/dbmigrate/import-postgresql/guide/mar-roles.sql
...
cd tools/phase1/dbmigrate/import-postgresql/guide/mar-db-client
