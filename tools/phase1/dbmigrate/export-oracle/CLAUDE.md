...
10. **A rendering bug found by import-oracle is fixed here** (`migration\dialects\oracle.ps1`), then the export is repeated and handed
    off again; never patch import-oracle's `input\`. The rules below are proven by import-oracle for the current data (see "What
    import-oracle has proven"); a new rendering path is proven only once import-oracle has loaded it.
...
## What import-oracle has proven

Windows can prove determinism, sanitizing, counts and that the text follows the rules above; it cannot prove that Oracle accepts it.
import-oracle loaded the current files unchanged into Oracle AI Database 26ai Free 23.26.3 (2026-09-24; 86 of 86 checks, 155 of 155 rows
identical) and **confirmed** without a correction here: `RESTART START WITH` inside `ALTER TABLE ... MODIFY (... GENERATED BY DEFAULT AS
IDENTITY (...))` (the next id is `identityLast + 1`; the `START WITH LIMIT VALUE` fallback is not needed); multi-row `INSERT ... VALUES`,
`BOOLEAN` and `DEFAULT FALSE NOT NULL`; `TIMESTAMP(3)` literals; the `WHENEVER SQLERROR` lines (a failure stops SQL*Plus with exit 1) and
`SET DEFINE OFF`; `timestamp`, `action`, `state`, `text`, `name`, `description` as unquoted column names (none reserved); the function-based
unique index; `CHR`/`UNISTR` pieces joined with `||`, including a surrogate-pair emoji (import-oracle's round-trip test). The database has
`MAX_STRING_SIZE = STANDARD`: a `VARCHAR2(2000 CHAR)` value is limited to 4,000 bytes. **Still unexercised by the current data**, so the
first suspects if a future load fails: `TO_CLOB(...)` literals (every CLOB value is NULL) and expressions spread over several lines (the
longest data line is 306 characters); the stress test in "Testing changes", loaded by import-oracle, covers both. A correction goes into
