# selftest: proves the export tooling is trustworthy (tools/phase1/dbmigrate/export-oracle/CLAUDE.md, "How selftest works"). Needs the live source (LocalDB) and a
...
function Invoke-SelfTest($Config, [string]$Target) {
    ...
    $work = Join-Path ([System.IO.Path]::GetTempPath()) ("mar-selftest-oracle-" + [guid]::NewGuid().ToString('N'))
    ...
    try {
        ...
        # Oracle rules the dialect must keep (see dialects\oracle.ps1)
        $lim = $script:OraMaxLineChars
        $longLines = 0; $highBytes = 0
        foreach ($f in @($paths.Schema, $paths.Data)) {
            foreach ($by in [System.IO.File]::ReadAllBytes($f)) { if ($by -gt 127) { $highBytes++ } }
            foreach ($ln in ([System.IO.File]::ReadAllText($f, $script:Utf8NoBom) -split "`n")) { if ($ln.Length -gt $lim) { $longLines++ } }
        }
        [void]$tests.Add((New-Check 'Oracle' "No line in the SQL files is longer than $lim characters (SQL*Plus stops at about 2,499)" ($longLines -eq 0) "$longLines line(s)"))
        [void]$tests.Add((New-Check 'Oracle' 'The SQL files are pure ASCII (control characters as CHR, other characters as UNISTR)' ($highBytes -eq 0) "$highBytes byte(s) above 127"))
        $idProblems = Ora-CheckIdentifiers $exp.Model
        [void]$tests.Add((New-Check 'Oracle' 'Every identifier is lowercase snake_case, at most 128 bytes and not an Oracle reserved word' ($idProblems.Count -eq 0) ($idProblems -join '; ')))
        $emptyValues = 0
        foreach ($t in $exp.Model.Tables) {
            for ($i = 0; $i -lt $t.Columns.Count; $i++) {
                if ($t.Columns[$i].Kind -ne 'string') { continue }
                foreach ($r in $exp.Rows[$t.Name]) { if ($null -ne $r[$i] -and ([string]$r[$i]).Length -eq 0) { $emptyValues++ } }
            }
        }
        [void]$tests.Add((New-Check 'Oracle' 'No source string value is empty (Oracle would store NULL instead)' ($emptyValues -eq 0) "$emptyValues empty value(s)"))
