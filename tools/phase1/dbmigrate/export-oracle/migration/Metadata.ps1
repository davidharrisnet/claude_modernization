# metadata: builds source-metadata.json, the data about the SQL Server source recorded at export (tools/phase1/dbmigrate/export-oracle/CLAUDE.md, "Contracts").
...
function New-SourceMetadata($Config, $Settings, $Model, $RowsByTable, [string]$SchemaText, [string]$DataText, $Summaries, [string]$SourceVersion, [string]$RunTimeUtc) {
    ...
    foreach ($t in $Model.Tables) {
        ...
        foreach ($c in $t.Columns) {
            ...
            [void]$cols.Add([ordered]@{
                ...
                targetName = (Ora-Column $t.Name $c.Name); targetType = (Ora-ColumnType $c); dataType = $info.DataType; maxLength = $info.MaxLength; precision = $info.Precision; scale = $info.Scale
                nullable = [bool]$c.Nullable; default = (Ora-DefaultText $c); identity = [bool]$c.IsIdentity
        ...
        foreach ($fk in ($t.ForeignKeys | Sort-Object { Ora-FkName $_ })) {
            [void]$fks.Add([ordered]@{
                targetName = (Ora-FkName $fk); sourceName = $fk.Name
                columns = [string[]]@($fk.Columns | ForEach-Object { Ora-Column $fk.Table $_ })
                refTable = (Ora-Table $fk.RefTable); refColumns = [string[]]@($fk.RefColumns | ForEach-Object { Ora-Column $fk.RefTable $_ })
                onDelete = [string]$fk.OnDelete
            })
        }
        $ixs = New-Object System.Collections.ArrayList
        foreach ($ix in ($t.Indexes | Sort-Object { Ora-IndexName $_.TargetName })) {
            $ixCols = New-Object System.Collections.ArrayList
            foreach ($ic in $ix.Columns) {
               [void]$ixCols.Add([ordered]@{ column = (Ora-Column $ix.Table $ic.Name); expression = (Ora-IndexColumnExpression $ix $ic $ci); descending = [bool]$ic.Descending })
            }
            [void]$ixs.Add([ordered]@{
                targetName = (Ora-IndexName $ix.TargetName); sourceName = $ix.Name; unique = [bool]$ix.Unique
                columns = [object[]]@($ixCols)
                filter = $(if ($ix.Filter) { Ora-TranslateFilter $ix.Table $ix.Filter } else { $null })
            })
        }
        [void]$tables.Add([ordered]@{
            sourceName = $t.Name; targetName = (Ora-Table $t.Name)
            rowCount = $rows.Count
            identityLast = $(if ($t.Identity -and $null -ne $t.IdentityLast) { [int64]$t.IdentityLast } else { $null })
            rowSha256 = (Get-TableRowSha256 $t $rows)
            columns = [object[]]@($cols)
            primaryKey = [string[]]@($t.PrimaryKey | ForEach-Object { Ora-Column $t.Name $_ })
    ...
    foreach ($t in $Model.Tables) {
        $colMap = [ordered]@{}
        foreach ($c in $t.Columns) { $colMap[$c.Name] = (Ora-Column $t.Name $c.Name) }
        $renameTables[$t.Name] = [ordered]@{ table = (Ora-Table $t.Name); columns = $colMap }
    }
    ...
    $doc = [ordered]@{
        ...
        knownDifferences = [string[]]@(Ora-KnownDifferences)
