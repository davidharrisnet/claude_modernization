# export-oracle entry point. Normally called through tools\phase1\dbmigrate\export-oracle\export-oracle.cmd:
#   export-oracle export|selftest|report|all --target oracle [--config path] [--out file]
...
function Show-Usage {
    Write-Host @'
Usage: export-oracle <command> --target <name> [options]

Commands:
  export    SQL Server -> 01-schema.sql, 02-data-sanitized.sql, source-metadata.json (Oracle AI Database 26ai; credentials sanitized)
  ...
  report    build the Word export report (MigrationExportReport.docx) from source-metadata.json
  all       export, selftest, report

Options:
  --target <name>   target block in the config file (required; oracle)
  --config <path>   config file (default tools\phase1\dbmigrate\export-oracle\migration\migration.config.json)
...
function Read-CliArgs([object[]]$Tokens) {
    ...
    for ($i = 0; $i -lt $Tokens.Count; $i++) {
        ...
        switch -Regex ($t) {
            ...
            '^-' { throw (New-MigrationError "Unknown option '$t'. Run 'export-oracle --help'." 2) }
...
try {
    ...
    switch ($cli.Command) {
        ...
        default { throw (New-MigrationError "Unknown command '$($cli.Command)'. Run 'export-oracle --help'." 2) }
    }
    exit $exit
} catch {
    $ex = $_.Exception
