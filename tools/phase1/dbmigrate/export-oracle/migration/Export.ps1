# export: SQL Server -> <outputDir>\<outputSubdir>\01-schema.sql, 02-data-sanitized.sql, source-metadata.json (Oracle AI Database 26ai).
# This tool is export only: nothing here talks to an Oracle server.
...
function Invoke-Export($Config, [string]$Target, [string]$OutDir, [bool]$Quiet) {
    ...
    # Credentials are always sanitized: this tool refuses to write anything else (docs/phase1/dbmigrate/DATA_MIGRATION.md 5.2).
    if (-not $settings.sanitizeCredentials) { throw (New-MigrationError "Target '$Target' has sanitizeCredentials off. export-oracle refuses to export unsanitized data; set it to true." 2) }
