# Shared helpers for the export-oracle tool (dot-sourced by DbMigrate.ps1). Trimmed to what a
# SQL Server -> Oracle file export needs (no target-database client, no Docker). Deterministic by design: sorted metadata,
# invariant culture, UTF-8 without BOM, LF newlines, no clock values in anything that is exported (except metadata.run).
...
"@
    ...
    # Index names are schema-global in Oracle but only table-scoped in SQL Server
...
function New-Check([string]$Category, [string]$Name, [bool]$Passed, [string]$Detail) {
    return [pscustomobject]@{ Category = $Category; Name = $Name; Passed = $Passed; Detail = $Detail }
}
...
# SQL Server datetime has millisecond precision, so its canonical text 'yyyy-MM-dd HH:mm:ss.fffffff' always has zeros after the
# 3rd fractional digit. Truncate to 3 digits for Oracle and the metadata; refuse to lose real precision.
function ConvertTo-Ts3([string]$Canonical) {
    if ($Canonical.Length -ne 27) { throw (New-MigrationError "Unexpected canonical timestamp '$Canonical'." 2) }
    if ($Canonical.Substring(23) -notmatch '^0+$') { throw (New-MigrationError "Timestamp '$Canonical' has sub-millisecond precision; Oracle TIMESTAMP(3) would lose it." 2) }
    return $Canonical.Substring(0, 23)
}
