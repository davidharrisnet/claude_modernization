...
# This tool verifies nothing against a target, so this is an export report, not a verification report (that is import-oracle's).
...
function Build-ExportReportDocument($M, $Images) {
    ...
    & $add (Pg (Rn 'MasterAntiqueRepair: SQL Server to Oracle') 'Subtitle')
    & $add (Pg (Rn 'Sanitized Oracle schema and data export' $true '2563EB' 28) '' '' $false 200)
    & $add (Banner $ok $(if ($ok) { 'EXPORT COMPLETE' } else { 'EXPORT INCOMPLETE' }) "$($tables.Count) tables  |  $totalRows rows  |  credentials sanitized for $($M.expectations.usersCount) users  |  Oracle AI Database 26ai (version $($M.meta.minOracleVersion)+)")
    ...
    & $add (Bullet 'The Oracle schema and data files are then written from that same sanitized in-memory copy. Identifiers become lowercase snake_case; data values are never lowercased.')
    & $add (Bullet 'The metadata file records what the source looked like at export: structure, row counts, a SHA-256 per table over the sanitized rows, the answers to ten business questions, and the SHA-256 of the two SQL files. It is computed from the in-memory rows, not from the SQL text, so it is an independent record that import-oracle can check the loaded database against.')
    ...
    & $add (Table @(1700, 1700, 1000, 1300, 3660) @('Source table', 'Oracle table', 'Rows', 'Last identity', 'Row hash (SHA-256, first 16)') $rows)
    & $add (Pg (Rn 'The hash is computed over all rows of the table in a fixed, database-independent form (Appendix A lists the full values). import-oracle recomputes it from the Oracle database it builds. An empty table hashes the empty string (e3b0c442...).' $false $null 0 $true) 'Caption')

    # ---- 5 schema
    & $add (PT '5. Schema' 'Heading1')
    & $add (PT 'Identifiers are lowercase snake_case in the SQL; Oracle stores them in upper case. Column types are the Oracle types the schema file declares. The users table also carries one column that does not exist in the source: must_reset_password (see section 7).')
    foreach ($t in $tables) {
        ...
        & $add (Table @(3060, 1700, 1100, 1200, 2300) @('Column (source -> Oracle)', 'Type', 'Nullable', 'Default', 'Notes') $crows)
    }
    ...
    & $add (PT 'ix_users_name_active is the important one: Oracle has no partial index, so it is a unique function-based index on CASE WHEN deleted_at IS NULL THEN LOWER(name) END; rows with a deleted_at value produce no key. An account that is soft-deleted keeps its row but frees its user name for reuse, and "Bob" and "bob" cannot both be active. The Filter column above shows the rule the key expression implements.')

    # ---- 6 business summaries
    & $add (PageBreak)
    & $add (PT '6. Business-level summaries' 'Heading1')
    & $add (PT 'Ten business questions were asked of the SQL Server source; the answers are recorded in the metadata so import-oracle can ask the same questions of the Oracle database and compare.')
    ...
    foreach ($e in @($M.excludedTables)) { & $add (Bullet "The Entity Framework migration history table ($e) is not exported; it only describes the SQL Server schema history and has no meaning in Oracle.") }
    ...
    & $add (Pg (Rn 'tools\phase1\dbmigrate\export-oracle\export-oracle.cmd all --target oracle' $true '1F3864') '' '' $false 40)
    ...
    & $add (PT 'Hand-off to import-oracle: after a successful export, copy 01-schema.sql, 02-data-sanitized.sql and source-metadata.json into tools\phase1\dbmigrate\import-oracle\input\ and commit them. import-oracle (Linux, Docker) reads only that folder. See docs/phase1/dbmigrate/import-oracle/README.md.')
...
function New-ExportReport([string]$MetaPath, [string]$OutFile) {
    ...
    Save-Docx $OutFile $body $images "Commit $($m.run.toolGitCommit) | $($m.run.runTimeUtc) UTC" $created 'MasterAntiqueRepair - export-oracle - Database Export' 'Database Export Report'
    return $OutFile
}
