# export-postgresql entry point. Normally called through tools\phase1\dbmigrate\export-postgresql\export-postgresql.cmd:
#   export-postgresql export|selftest|report|all --target postgres [--config path] [--out file]
...
function Show-Usage {
    Write-Host @'
Usage: export-postgresql <command> --target <name> [options]
...
Options:
  ...
  --config <path>   config file (default tools\phase1\dbmigrate\export-postgresql\migration\migration.config.json)
...
function Read-CliArgs([object[]]$Tokens) {
    ...
    for ($i = 0; $i -lt $Tokens.Count; $i++) {
        ...
        switch -Regex ($t) {
            ...
            '^-' { throw (New-MigrationError "Unknown option '$t'. Run 'export-postgresql --help'." 2) }
...
try {
    ...
    switch ($cli.Command) {
        ...
        default { throw (New-MigrationError "Unknown command '$($cli.Command)'. Run 'export-postgresql --help'." 2) }
    }
    exit $exit
} catch {
    $ex = $_.Exception
