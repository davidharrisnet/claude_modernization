...
# This tool is export only: nothing here talks to a PostgreSQL server.
...
function Invoke-Export($Config, [string]$Target, [string]$OutDir, [bool]$Quiet) {
    ...
    if (-not $settings.sanitizeCredentials) { throw (New-MigrationError "Target '$Target' has sanitizeCredentials off. export-postgresql refuses to export unsanitized data; set it to true." 2) }
