# export-postgresql: SQL Server -> sanitized PostgreSQL export (Windows)
...
- Description and latest results: `docs/phase1/dbmigrate/export-postgresql/README.md`. Instructions for Claude Code: `CLAUDE.md` in this folder.
- The Linux side that loads and verifies these files is import-postgresql: `docs/phase1/dbmigrate/import-postgresql/README.md`.
...
| `report` | Builds `docs\phase1\dbmigrate\export-postgresql\MigrationExportReport.docx` from `source-metadata.json` |
...
## Hand-off to import-postgresql

After a successful export, copy `01-schema.sql`, `02-data-sanitized.sql` and `source-metadata.json` into `tools\phase1\dbmigrate\import-postgresql\input\` and commit both folders. import-postgresql reads only its own `input\` folder.
