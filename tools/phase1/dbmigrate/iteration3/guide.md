...
| `docker rmi masterantique-sqlite:iteration3` | The image itself, and the baked-in data, are gone. Rebuild from `tools/phase1/dbmigrate/iteration3/` to recreate it. |
...
From `tools/phase1/dbmigrate/iteration3/` on a Linux host with Docker (and a copy of the raw `02-data.sql` from `tools/phase1/dbmigrate/iteration2/02-data.sql` — it's gitignored here and regenerated/copied locally, never committed):
```
...
```
pandoc guide.md -o ../../../../docs/phase1/dbmigrate/iteration3/SQLiteDatabaseGuide3.docx
```
