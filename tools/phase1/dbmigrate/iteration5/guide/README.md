...
`docs/phase1/dbmigrate/iteration5/PostgreSQLDatabaseGuide.html` is generated from this folder. **Every code block in the guide
...
```
python3 tools/phase1/dbmigrate/iteration5/guide/build-guide.py
```
...
sed -e 's/^CONTAINER=.*/CONTAINER=mar-postgres-guide/' \
    -e "s|^INPUT_DIR=.*|INPUT_DIR=$PWD/tools/phase1/dbmigrate/iteration5/input|" \
    tools/phase1/dbmigrate/iteration5/ingest.conf.example > /tmp/guide.conf
tools/phase1/dbmigrate/iteration5/ingest.sh load --config /tmp/guide.conf
...
docker exec -i -e APP_PW -e RO_PW mar-postgres-guide psql -X -q -U masterantique -d masterantique \
  < tools/phase1/dbmigrate/iteration5/guide/mar-roles.sql
...
cd tools/phase1/dbmigrate/iteration5/guide/mar-db-client
