# Iteration 5 — instructions for Claude Code

This folder is the iteration 5 tool: it loads iteration 4's PostgreSQL export into a PostgreSQL container on Linux
and verifies it against the export's record of the source. The human description is
`docs/phase1/dbmigrate/iteration5/README.md`; the strategy and security policy for all iterations is
`docs/phase1/dbmigrate/DATA_MIGRATION.md` (§5 security, §8 decisions for iterations 4 and 5).

## Rules

1. **Never edit `input/` by hand.** It holds iteration 4's three output files. To refresh them, re-export on Windows
   (iteration 4) and copy all three together.
2. **Never fix rendering bugs here.** If the SQL fails to load or a hash disagrees because of how iteration 4 wrote
   the SQL, fix `tools/phase1/dbmigrate/iteration4/migration/dialects/postgres.ps1` and re-export.
...
6. **Generated files are never edited by hand**: `verification-results.json`, `selftest-results.json`,
   `docs/.../MigrationVerificationReport5.html`, `docs/.../PostgreSQLDatabaseGuide.html` (see `guide/README.md`).
...
counts usually means iteration 4 and `verify.sql` disagree on the canonical form (empty-table hash, 3-digit
...
`docs/phase1/dbmigrate/iteration5/MigrationVerificationReport5.html`. Self-contained HTML (inline CSS, light/dark,
...
## Input contract with iteration 4

If iteration 4 changes any of this, `verify.sql` changes with it.
...
`docs/phase1/dbmigrate/iteration5/PostgreSQLDatabaseGuide.html` (also published at
...
```
sed -e 's/^CONTAINER=.*/CONTAINER=mar-postgres-test/' -e "s|^INPUT_DIR=.*|INPUT_DIR=$PWD/tools/phase1/dbmigrate/iteration5/input|" \
    tools/phase1/dbmigrate/iteration5/ingest.conf.example > /tmp/test.conf
tools/phase1/dbmigrate/iteration5/ingest.sh load --config /tmp/test.conf
tools/phase1/dbmigrate/iteration5/ingest.sh verify --config /tmp/test.conf --out /tmp/test-results.json
