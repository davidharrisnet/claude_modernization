...
function Build-GuideDocument($Settings, [string]$Db, [string]$DisplayPath, $Results, [string]$Target, [string]$HostFile, $ExportPaths = $null) {
    ...
    if ($docker) {
        ...
        & $add (PT "Change the scratch copy freely, then delete it. To return the real database to its migrated state, rebuild it from the exported files (this deletes the database in the container and recreates it): tools\dbmigrate\iteration2\dbmigrate.cmd import --target $Target --recreate")
        ...
        & $add (PT "The container was created from the stock alpine:3.20 image; SQLite was installed into it with apk add sqlite sqlite-tools. There is no custom image and no volume, so the container is the only home of the database. To rebuild everything: tools\dbmigrate\iteration2\dbmigrate.cmd all --target $Target --recreate")
        ...
        & $add (Table @(3300, 6060) @('Symptom', 'Cause and fix') @(
            @('Error: No such container', "The container was removed. Run the pipeline again to recreate it and the database: tools\dbmigrate\iteration2\dbmigrate.cmd all --target $Target"),
    ...
    & $add (Code "tools\dbmigrate\iteration2\dbmigrate.cmd all --target $Target --recreate")
    ...
    & $add (Code "tools\dbmigrate\iteration2\dbmigrate.cmd guide --target $Target")
