...
function global:Sqlite-KnownDifferences($Settings) {
    ...
    if (Sqlite-IsDocker $Settings) {
        $list += 'The database was created and checked by the Linux build of SQLite running in a Docker container (Alpine Linux); the native Windows target (iteration 1) used a different SQLite version. The SQLite file format is identical on both, so a file built on one platform opens on the other. The database file lives only inside the container (/data); no copy is taken out.'
    }
    return $list
}
