# Shared helpers for the iteration 4 export tool (dot-sourced by DbMigrate.ps1). Copied from iteration 2 and trimmed to what a
# SQL Server -> PostgreSQL file export needs (no target-database client, no Docker). Deterministic by design: sorted metadata,
...
# 3rd fractional digit. Truncate to 3 digits for PostgreSQL and the metadata; refuse to lose real precision.
function ConvertTo-Ts3([string]$Canonical) {
    ...
    if ($Canonical.Substring(23) -notmatch '^0+$') { throw (New-MigrationError "Timestamp '$Canonical' has sub-millisecond precision; PostgreSQL TIMESTAMP(3) would lose it." 2) }
