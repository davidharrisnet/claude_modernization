# export report content: builds MigrationExportReport4.docx from source-metadata.json only (helpers are in Report.ps1).
# Iteration 4 verifies nothing against a target, so this is an export report, not a verification report (that is iteration 5's).
...
function Build-ExportReportDocument($M, $Images) {
    ...
    & $add (Pg (Rn 'Iteration 4 - Sanitized PostgreSQL schema and data export' $true '2563EB' 28) '' '' $false 200)
    ...
    & $add (PT 'This iteration only exports. It does not load or check a PostgreSQL database; that is iteration 5, which reads these files on a Linux machine, creates a PostgreSQL database in a Docker container, and verifies it against the metadata described here. Everything in this report was produced by a deterministic script, not by a language model.')
    ...
    & $add (Bullet 'The metadata file records what the source looked like at export: structure, row counts, a SHA-256 per table over the sanitized rows, the answers to ten business questions, and the SHA-256 of the two SQL files. It is computed from the in-memory rows, not from the SQL text, so it is an independent record that iteration 5 can check the loaded database against.')
    ...
    & $add (Pg (Rn 'The hash is computed over all rows of the table in a fixed, database-independent form (Appendix A lists the full values). Iteration 5 recomputes it from the PostgreSQL database it builds. An empty table hashes the empty string (e3b0c442...).' $false $null 0 $true) 'Caption')
    ...
    & $add (PT 'Ten business questions were asked of the SQL Server source; the answers are recorded in the metadata so iteration 5 can ask the same questions of the PostgreSQL database and compare.')
    ...
    & $add (PT 'Every users row had its password hash and security stamp set to NULL, and a new must_reset_password column set to true. This is a deliberate one-time-bootstrap policy: forcing a password reset at first login means no usable legacy credential is carried into the target at all (docs/DATA_MIGRATION.md 5.2). Nothing else in the users table, or in any other table, was changed.')
    ...
    & $add (Pg (Rn 'tools\dbmigrate\iteration4\dbmigrate4.cmd all --target postgres' $true '1F3864') '' '' $false 40)
    ...
    & $add (PT 'Hand-off to iteration 5: after a successful export, copy 01-schema.sql, 02-data-sanitized.sql and source-metadata.json into tools\dbmigrate\iteration5\input\ and commit them. Iteration 5 (Linux, Docker) reads only that folder. See docs/dbmigrate/iteration5/ITERATION5_PLAN.md.')
...
function New-ExportReport([string]$MetaPath, [string]$OutFile) {
    ...
    Save-Docx $OutFile $body $images "Commit $($m.run.toolGitCommit) | $($m.run.runTimeUtc) UTC" $created 'MasterAntiqueRepair - Iteration 4 - Database Export' 'Database Export Report'
    return $OutFile
}
