...
- Plan: `docs/dbmigrate/iteration4/ITERATION4_PLAN.md`. Record of the real run: `docs/dbmigrate/iteration4/ITERATION4.md`.
- The Linux side that loads and verifies these files is iteration 5: `docs/dbmigrate/iteration5/ITERATION5_PLAN.md`.
...
```
tools\dbmigrate\iteration4\dbmigrate4.cmd all --target postgres
```
...
| `report` | Builds `docs\dbmigrate\iteration4\MigrationExportReport4.docx` from `source-metadata.json` |
...
After a successful export, copy `01-schema.sql`, `02-data-sanitized.sql` and `source-metadata.json` into `tools\dbmigrate\iteration5\input\` and commit both folders. Iteration 5 reads only its own `input\` folder.
